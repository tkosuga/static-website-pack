...
//
// Usage:
//  const appEnv = require(`${process.cwd()}/gulp/website_env`)('example/config.js');
//  const config = appEnv.config;
//
// build出力先のbuildPathと成果物出力先のdistPathのフォルダが存在しない場合は新しくフォルダを作成します。
//
const file_copy = require(`${process.cwd()}/gulp/utils/file_copy`);
const http_server = require(`${process.cwd()}/gulp/utils/http_server`);
const mkdir = require(`${process.cwd()}/gulp/utils/mkdir`);
const load_config = require(`${process.cwd()}/gulp/utils/load_config`);
//
...
module.exports = (configAbsolutePath) => {
  ...
  return {
    config: config,
    task: (task) => require(`${process.cwd()}/gulp/tasks/${task}`)(config),
